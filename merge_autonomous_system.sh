...
pip install psutil rich cryptography numpy scikit-learn

cat > aegis_unified.py <<'PY'
#!/usr/bin/env python3
...
chmod +x aegis_unified.py

cat > run_unified.sh <<'RUN'
#!/usr/bin/env bash
...
cat > run_unified_all.sh <<'RUNALL'
#!/usr/bin/env bash
printf "all\nexit\n" | "$HOME/aegis_omni_xeon/run_unified.sh"
RUNALL

chmod +x run_unified_all.sh

echo "AEGIS Unified installed."
echo "Interactive: ~/aegis_omni_xeon/run_unified.sh"
echo "Run all:     ~/aegis_omni_xeon/run_unified_all.sh"
